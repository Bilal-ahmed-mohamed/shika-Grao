...
<ul class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-y-12 gap-x-8 p-8">
  {turfs.map((fetchedData) => (
    <li class="relative flex flex-col md:flex-row  rounded-lg shadow-lg p-6 w-full">
      <img src={fetchedData.Image} alt="" class="mb-4 sm:mb-0 sm:mr-6 xl:mb-4 xl:mr-0 rounded-lg object-cover w-full md:w-[15rem] " />
      <div class="flex flex-col justify-between flex-1 mx-10">
        ...
        <Link  to={`/IndivualTurf/${fetchedData.turf_id}`} class="mt-6 inline-flex items-center justify-center h-10 rounded-full text-sm font-semibold bg-blue-600 text-white px-5 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
          Book Now
        </Link>
      </div>
    </li>
